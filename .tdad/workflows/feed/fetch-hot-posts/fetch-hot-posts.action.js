/**
 * Fetch Hot Posts Action
 *
 * Retrieves posts sorted by hot score from the API.
 ...
 */
async function performFetchHotPostsAction(page, context = {}) {
    try {
...
/**
 * Helper to create a test post for hot posts tests
 * @param {Object} page - Playwright page object
...
async function createTestPost(page, options = {}) {
    try {
        ...
        const anonymousId = `anon_hotpost_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;

        const postData = {
            frustration: frustration || `Test frustration for hot posts ${Date.now()}`,
            identity: identity || `a hot posts tester`,
    ...
    try {
        ...
        for (let i = 0; i < count; i++) {
            const result = await createTestPost(page, {
                ...options,
                frustration: options.frustration || `Hot post test frustration ${Date.now()}_${i}`,
 ...
 * @param {Object} result - Result from performFetchHotPostsAction
...
/**
 * Helper to check if posts are sorted by hot score descending
...
function arePostsSortedByHotScore(posts) {
    ...
    for (let i = 0; i < posts.length - 1; i++) {
        const currentScore = posts[i].hotScore ?? 0;
        const nextScore = posts[i + 1].hotScore ?? 0;

        // Hot score should be descending
