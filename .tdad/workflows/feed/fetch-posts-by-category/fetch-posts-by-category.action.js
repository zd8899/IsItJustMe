/**
 * Fetch Posts By Category Action
 *
 * Retrieves posts filtered by category slug from the API.
 ...
 * @param {string} context.categorySlug - Category slug to filter by (required)
 * @param {number} context.limit - Number of posts to fetch (1-50, default 20)
 * @param {string} context.cursor - Cursor for pagination
 * @param {boolean} context.omitCategorySlug - If true, omit categorySlug from request
 * @returns {Promise<Object>} - Returns { success, errorMessage, statusCode, body, posts, nextCursor }
 */
async function performFetchPostsByCategoryAction(page, context = {}) {
    try {
        const { mode, categorySlug, limit, cursor, omitCategorySlug } = context;
        ...
        if (mode === 'api' || !mode) {
            ...
            if (!omitCategorySlug && categorySlug !== undefined) {
            ...
            let url = '/api/trpc/post.listByCategory';
            ...
            if (!response.ok()) {
                ...
                if (!errorMessage) {
                    errorMessage = body?.error?.json?.message || body?.error?.message || 'Fetch posts by category failed';
                }
            }
 ...
 * @param {Object} page - Playwright page object
 * @returns {Promise<Object>} - Returns { success, categories, errorMessage }
 */
async function getAllCategories(page) {
    try {
        ...
        return { success: false, categories: [], errorMessage: 'Failed to fetch categories' };
    } catch (error) {
        return { success: false, categories: [], errorMessage: error.message };
    }
}

/**
 * Helper to create a test post for category tests
 * @param {Object} page - Playwright page object
 * @param {Object} options - Options for creating the post
 * @param {string} options.categoryId - Category ID (required if categorySlug not provided)
 * @param {string} options.categorySlug - Category slug (used to find categoryId)
...
async function createTestPost(page, options = {}) {
    try {
        ...
        const anonymousId = `anon_catpost_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;

        const postData = {
            frustration: frustration || `Test frustration for category ${Date.now()}`,
            identity: identity || `a category tester`,
        ...
        return {
            ...
            categorySlug: body?.category?.slug || options.categorySlug || null,
...
/**
 * Helper to create multiple test posts
 * @param {Object} page - Playwright page object
    ...
    try {
        ...
        for (let i = 0; i < count; i++) {
            const result = await createTestPost(page, {
                ...options,
                frustration: options.frustration || `Category post test frustration ${Date.now()}_${i}`,
 ...
 * @param {Object} result - Result from performFetchPostsByCategoryAction
...
/**
 * Helper to check if posts are sorted by creation date descending
 * @param {Array} posts - Array of posts
 * @returns {boolean} - True if sorted correctly
 */
function arePostsSortedByCreatedAtDesc(posts) {
    ...
    for (let i = 0; i < posts.length - 1; i++) {
        ...
        // Created at should be descending (newer first)
