...
/**
 * Handle Downvote Click Action
 *
 * Processes downvote button clicks on posts and comments.
 * Handles new downvotes, toggle off (remove), and direction change (upvote to downvote).
 *
 ...
 */
async function performHandleDownvoteClickAction(page, context = {}) {
    try {
        const {
            mode,
            targetType = 'post',
        ...
        if (mode === 'api') {
            ...
            let voteStatus = -1; // Default: downvote is active

            if (voteResult.action === 'created') {
                // New downvote: score - 1
                newScore = initialScore - 1;
                voteStatus = -1;
            } else if (voteResult.action === 'deleted') {
                // Toggle off: score + 1 (existing downvote removed)
                newScore = initialScore + 1;
                voteStatus = null;
            } else if (voteResult.action === 'updated') {
                // Direction change from upvote to downvote: score - 2
        ...
        // ==========================================
        if (mode === 'ui') {
            return await performDownvoteClickUI(page, context);
        }
...
async function performDownvoteClickUI(page, context = {}) {
    try {
        ...
        // Find the appropriate downvote button
        let downvoteButton;
        let scoreDisplay;
        ...
        const downvoteClassesBefore = await downvoteButton.getAttribute('class') || '';
        const downvoteActiveBefore = downvoteClassesBefore.includes('text-red') ||
                                     downvoteClassesBefore.includes('text-orange');
        ...
        const downvoteClassesAfter = await downvoteButton.getAttribute('class') || '';
        const downvoteActiveAfter = downvoteClassesAfter.includes('text-red') ||
                                    downvoteClassesAfter.includes('text-orange');

        // Get upvote button state
        let upvoteButton;
        if (target === 'comment') {
        ...
        return {
            ...
            scoreAfter,
            newScore: scoreAfter,
            downvoteActiveBefore,
            downvoteActiveAfter,
            downvoteActive: downvoteActiveAfter,
            upvoteActive,
            errorMessage: null
        };
...
/**
 * Check if downvote button shows active state
 * @param {Object} page - Playwright page object
 ...
 */
async function checkDownvoteButtonState(page, context = {}) {
    try {
